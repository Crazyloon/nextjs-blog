...
const NavToggle = ({isOpen, setMenuOpen, icon}) => {
  ...
  return ( 
    <li ref={ref} className={`text-link-text absolute top-0 right-0 bg-navbar text-xl justify-center align-center hover:bg-secondary-hover cursor-pointer transition-colors m-0 float-right block ${isOpen ? 'float-none text-left' : 'lg:hidden'}`} onClick={() => setMenuOpen(!isOpen)}>
