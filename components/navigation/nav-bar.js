...
import { faHome, faTh, faBlog , faBars, faVideo, faBowlFood } from '@fortawesome/free-solid-svg-icons'
  ...
  return (
    <nav className='bg-navbar mb-8'>
      <ul className={`m-0 overflow-hidden bg-navbar list-none relative ${menuOpen ? 'overflow-visible' : ''}`}>
        ...
        <NavLink active={router.pathname === '/posts'} url='/posts' text='Articles' icon={faBlog}  display={menuOpen} />
        <NavLink active={router.pathname === '/vlog'} url='/vlog' text='Videos' icon={faVideo}  display={menuOpen} />
        <NavLink active={router.pathname === '/gallery'} url='/gallery' text='Gallery' icon={faTh}  display={menuOpen} />
        <NavLink active={router.pathname === '/recipes'} url='/recipes' text='Recipes' icon={faBowlFood}  display={menuOpen} />
